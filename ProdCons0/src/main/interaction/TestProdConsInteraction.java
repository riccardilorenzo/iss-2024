...
private static Interaction connSupport;
	...
	public void testRequest() {
		...
		IApplMessage req  = CommUtils.buildRequest( "riccardilorenzo", "distance", "distance(20)", "consumer");
		IApplMessage req1 = CommUtils.buildRequest( "riccardilorenzo", "distance", "distance(30)", "consumer");
 		try {
	...
	public void testDoubleRequest() {
		CommUtils.outmagenta("testDoubleRequest ======================================= ");
		//Funge da Producer come ProducerUsingConnection
		IApplMessage req  = CommUtils.buildRequest( "riccardilorenzo", "distance", "distance(20)", "consumer");
		IApplMessage disp = CommUtils.buildRequest( "riccardilorenzo", "distance", "distance(30)", "consumer");
		ArrayList<Integer> integers = new ArrayList<>(); integers.add(20); integers.add(30);
		ArrayList<IApplMessage> answers = new ArrayList<>();
 		try {
 			connSupport.forward(disp);
 			
			answers.add(connSupport.request(req));
			
			answers.add(connSupport.receive());
			
			answers.forEach(System.out::println);
			assertTrue(everyAckReceived(answers, integers));
	...
	protected boolean everyAckReceived(ArrayList<IApplMessage> acks, ArrayList<Integer> integers) {
		boolean res = false;
		
		for (IApplMessage ack : acks) {
			res = false;
			for (Integer num : integers) {
				if (ack.msgContent().contains(String.valueOf(num))) {
					integers.remove(num);
					res = true;
					break;
				}
